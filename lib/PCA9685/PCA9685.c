 ...
 */
//for MODE2
void PCA9685_InitOne(char addr){
    DEV_I2C_Init(addr);
    I2C_Write_Byte(MODE1, 0x00); //test with 0x01 first , then 0x00
}
//FOR MODE1
...
//MODE2
void PCA9685_SetPWMFreqOne(UWORD freq)
{
    freq *= 0.9;  // Correct for overshoot in the frequency setting (see issue #11).
    double prescaleval = 25000000.0;
    prescaleval /= 4096.0;
    ...
    UBYTE oldmode = PCA9685_ReadByte(MODE2);
    UBYTE newmode = (oldmode & 0x7F) | 0x10; // sleep

    PCA9685_WriteByte(MODE2, newmode); // go to sleep
    PCA9685_WriteByte(PRESCALE, prescale); // set the prescaler
    PCA9685_WriteByte(MODE2, oldmode);
    DEV_Delay_ms(5);
    PCA9685_WriteByte(MODE2, oldmode | 0x80);  //  This sets the MODE1 register to turn on auto increment.
}
